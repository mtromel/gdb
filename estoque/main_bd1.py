...
# Consulta SQL
cursor.execute('SELECT I.ITEMID, IT.ITEMNAME, D.INVENTLOCATIONID, D.WMSLOCATIONID, I.PHYSICALINVENT, P.UNITID, P.PRICE, I.PHYSICALINVENT * P.PRICE AS TOTAL '
      'FROM {TABLE_INVENTSUM} AS I '
      'LEFT JOIN {TABLE_INVENTDIM} AS D ON I.INVENTDIMID = D.INVENTDIMID '
      'LEFT JOIN {TABLE_INVENTTABLE} AS IT ON I.ITEMID = IT.ITEMID '
      'LEFT JOIN {TABLE_INVENTITEMPRICE} AS P ON I.ITEMID = P.ITEMID '
      'WHERE I.PHYSICALINVENT > 0 AND I.DATAAREAID = "gsb" '
      'AND D.DATAAREAID = "gsb" '
      'AND P.VERSIONID = " " '
      'AND P.PRICETYPE = "0" '
      'AND P.DATAAREAID = "gsb"'
      )
...
for row in cursor:
    ...
    sum_total += ttl
    data += [[item, name, dep, loc, fis_inv, unitid, price.value, ttl]]
    if dep.value == '01':
        dep01 += ttl
    elif dep.value == '02':
        dep02 += ttl
